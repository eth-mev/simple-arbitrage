...
export class Arbitrage {
  ...
  async takeProfitIfNeeded(bestCrossedMarket: CrossedMarketDetails,
                            blockNumber: number,
                            minerRewardPercentage: number,
                            block: Block,
                            chainId: number,
                            gasPrice: BigNumber): Promise<void> {
    ...
    try {
      ...
      const netProfit = bestCrossedMarket.profit.sub(gasPrice.mul(estimateGas));

      if (netProfit.lt(0)) {
        log(`netProfit: ${netProfit}`, logStatus.estimateFail);
        console.warn(`Estimate gas failure for netProfit: ${netProfit}`)
        return
      }
    ...
    log(`${bundleSubmitResponse}`, logStatus.bundleSubmitResponse)
    console.log(`bundleSubmitResponse: ${bundleSubmitResponse}`);


    if ('error' in bundleSubmitResponse) {
      log(`${bundleSubmitResponse.error.message}`, logStatus.bundleSubmitResponseError)
  ...
  async takeCrossedMarkets(bestCrossedMarkets: CrossedMarketDetails[],
    ...
    for (const bestCrossedMarket of bestCrossedMarkets) {
      this.takeProfitIfNeeded(bestCrossedMarket,
        blockNumber,
        minerRewardPercentage,
        block,
        chainId,
        gasPrice)
        .catch(console.error)
